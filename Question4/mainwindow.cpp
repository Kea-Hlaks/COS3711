...
const QString MainWindow::XML_FILE = "students.xml";
    ...
    , m_process(new QProcess(this))
{
    ...
    StudentList::instance()->loadFromXml(
        QCoreApplication::applicationDirPath() + "/" + XML_FILE);
}
...
{
    // Save list to XML — safe to call even when the list is empty
    StudentList::instance()->saveToXml(
        QCoreApplication::applicationDirPath() + "/" + XML_FILE);
    event->accept();
}
...
void MainWindow::onGraduateClicked()
{
    ...
    m_resultLabel->setText(
        number + (s->graduate() ? " qualifies for graduation."
                                : " does NOT qualify for graduation."));
}
